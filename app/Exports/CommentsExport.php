...
class CommentsExport implements FromCollection, WithHeadings, WithEvents
{
    protected $filters;
    ...
    public function collection()
    {
        ...
        if (!empty($this->filters['user'])) {
            ...
            $q->whereHas('user', function ($u) use ($value) {
                $u->whereRaw('LOWER(name) LIKE ?', ['%' . $value . '%']);
            });
        }
        ...
        return $q->get()->map(function ($c) {
            return [
                'ID'          => $c->id,
                'Usuario'     => $c->user->name ?? 'Usuario eliminado',
                'Descripción' => $c->description,
                'Fecha'       => $c->created_at->format('Y-m-d H:i:s'),
            ];
        });
    }
    ...
    {
        return ['ID', 'Usuario', 'Descripción', 'Fecha'];
    }
